...
from .backbone_resnet_sn import ResNetBackbone, Bottleneck
...
class RandomFeatureGaussianProcess(nn.Module):
    def __init__(
            self,
            out_features: int,
            backbone: nn.Module = ResNetBackbone(inplanes = 2,
                                                 block = Bottleneck,
                                                 layers = [3, 4, 6, 3]),
        ...
        # 512 is the number of features in resnet hidden layers
        random_fourier_feature_layer = nn.Linear(512, num_inducing)
